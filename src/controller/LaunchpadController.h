...
class LaunchpadPadState
{
    public:
        ...
        PushState pushState() const     { return m_PushState; }
        LaunchpadInterface::Color color() const  { return m_Color;     }
        int note() const                { return m_Note;      }
        int velocity() const            { return m_Velocity;  }

        void setPushState(const PushState state)    { m_PushState = state;   }
        void setColor(const LaunchpadInterface::Color color) { m_Color = color;       }
        void setNote(const int note)                { m_Note = note;         }
